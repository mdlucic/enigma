...
class UserAuth
{
	...
	public function saveData(): bool
	{
		if ($this->isValidInput() == true) {
			$sql = "INSERT INTO user_profile (first_name, last_name, username, email, password, status, registration_date, verification_code) VALUES (:first_name, :last_name, :username, :email, :password, :status, :registration_date, :verification_code)";
	...
	{
		$sql = "SELECT * FROM user_profile WHERE verification_code = :verification_code";
		$stmt = $this->connection->prepare($sql);
		$stmt->bindParam(':verification_code', $this->verification_code);
		$stmt->execute();
	...
	public function createPage(string $username)
	{
		...
		if ($this->enableAccount() == true) {
			...
			if (isset($user_profile_page)) {
				$txt = "<?php
					$user_profile_id = $username_string;
